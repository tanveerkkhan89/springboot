    ...
    Statement = [
      {
        Effect = "Allow",
        Principal = {
          Federated = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/${data.aws_eks_cluster.eks_cluster.identity[0].oidc_provider}"
        },
        Action = "sts:AssumeRoleWithWebIdentity",
        Condition = {
          StringEquals = {
            "${data.aws_eks_cluster.eks_cluster.identity[0].oidc_provider}:sub": "system:serviceaccount:kube-system:alb-ingress-controller"
          }
        }
      }
    ]
